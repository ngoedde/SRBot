...
public class Blowfish
{
    ...
    private void Blowfish_encipher(ref uint xl, ref uint xr)
    {
        ...
        Xl ^= PArray[0];
        this.ROUND(ref Xr, Xl, 1); this.ROUND(ref Xl, Xr, 2);
        this.ROUND(ref Xr, Xl, 3); this.ROUND(ref Xl, Xr, 4);
        this.ROUND(ref Xr, Xl, 5); this.ROUND(ref Xl, Xr, 6);
        this.ROUND(ref Xr, Xl, 7); this.ROUND(ref Xl, Xr, 8);
        this.ROUND(ref Xr, Xl, 9); this.ROUND(ref Xl, Xr, 10);
    ...
    private void Blowfish_decipher(ref uint xl, ref uint xr)
    {
        ...
        this.ROUND(ref Xr, Xl, 10); this.ROUND(ref Xl, Xr, 9);
        this.ROUND(ref Xr, Xl, 8); this.ROUND(ref Xl, Xr, 7);
        this.ROUND(ref Xr, Xl, 6); this.ROUND(ref Xl, Xr, 5);
        this.ROUND(ref Xr, Xl, 4); this.ROUND(ref Xl, Xr, 3);
        this.ROUND(ref Xr, Xl, 2); this.ROUND(ref Xl, Xr, 1);
        Xr ^= PArray[0];
    ...
    public void Initialize(byte[] key_ptr, int offset, int length)
    {
        ...
        for (i = 0; i < 16 + 2; i += 2)
        {
            this.Blowfish_encipher(ref datal, ref datar);
            PArray[i] = datal;
            PArray[i + 1] = datar;
        }
    ...
    public byte[] Encode(byte[] stream, int offset, int length)
    {
        ...
        byte[] workspace = new byte[this.GetOutputLength(length)];
        ...
        for (int x = 0; x < workspace.Length; x += 8)
        {
            ...
            this.Blowfish_encipher(ref l, ref r);
    ...
    public byte[] Decode(byte[] stream, int offset, int length)
    {
        ...
        for (int x = 0; x < workspace.Length; x += 8)
        {
            uint l = BitConverter.ToUInt32(workspace, x + 0);
            uint r = BitConverter.ToUInt32(workspace, x + 4);
            this.Blowfish_decipher(ref l, ref r);
            Buffer.BlockCopy(BitConverter.GetBytes(l), 0, workspace, x + 0, 4);
            Buffer.BlockCopy(BitConverter.GetBytes(r), 0, workspace, x + 4, 4);
        }
