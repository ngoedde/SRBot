...
public class Security
{
    ...
    private void SetupCountByte(uint seed)
    {
        ...
        uint mut = seed;
        uint mut1 = this.GenerateValue(ref mut);
        uint mut2 = this.GenerateValue(ref mut);
        uint mut3 = this.GenerateValue(ref mut);
        this.GenerateValue(ref mut);
    ...
    private void Handshake(ushort packet_opcode, PacketReader packet_data, bool packet_encrypted)
    {
        ...
        if (m_client_security)
        {
            ...
            m_value_K = this.G_pow_X_mod_P(m_value_p, m_value_x, m_value_B);

            key_array = MAKELONGLONG_(m_value_A, m_value_B);
            this.KeyTransformValue(ref key_array, m_value_K, (byte)(LOBYTE_(LOWORD_(m_value_K)) & 0x03));
            ...
            this.KeyTransformValue(ref key_array, m_value_K, (byte)(LOBYTE_(LOWORD_(m_value_B)) & 0x07));
            ...
            this.KeyTransformValue(ref m_challenge_key, m_value_K, (byte)(LOBYTE_(LOWORD_(m_value_A)) & 0x07));
            tmp_bytes = m_blowfish.Encode(BitConverter.GetBytes(m_challenge_key));
            m_challenge_key = BitConverter.ToUInt64(tmp_bytes, 0);

            this.KeyTransformValue(ref m_handshake_blowfish_key, m_value_K, 0x3);
        ...
        else
        {
            ...
            if (flags.handshake == 1)
            {
                ...
                m_value_B = this.G_pow_X_mod_P(m_value_p, m_value_x, m_value_g);
                m_value_K = this.G_pow_X_mod_P(m_value_p, m_value_x, m_value_A);
                ...
                this.KeyTransformValue(ref m_client_key, m_value_K, (byte)(LOBYTE_(LOWORD_(m_value_B)) & 0x07));
            ...
            if (flags.handshake_response == 1)
            {
                ...
                this.KeyTransformValue(ref expected_challenge_key, m_value_K,
                    (byte)(LOBYTE_(LOWORD_(m_value_A)) & 0x07));
    ...
    private byte[] FormatPacket(ushort opcode, byte[] data, bool encrypted)
    {
        ...
        if (m_client_security == false && m_security_flags.security_bytes == 1)
        {
            ...
            byte sb1 = this.GenerateCountByte(true);
            writer.BaseStream.Seek(4, SeekOrigin.Begin);
            writer.Write(sb1);
            writer.Flush();

            byte sb2 = this.GenerateCheckByte(writer.GetBytes());
    ...
    private KeyValuePair<TransferBuffer, Packet> GetPacketToSend()
    {
        ...
        if (packet.Massive)
        {
            ...
            final.Write(this.FormatPacket(0x600D, final_header.GetBytes(), false));
            ...
            packet.Lock();
            return new KeyValuePair<TransferBuffer, Packet>(new TransferBuffer(raw_bytes, 0, raw_bytes.Length, true),
                packet);
        }
        else
        {
            ...
            byte[] raw_bytes = this.FormatPacket(packet.Opcode, packet.GetBytes(), encrypted);
            packet.Lock();
            return new KeyValuePair<TransferBuffer, Packet>(new TransferBuffer(raw_bytes, 0, raw_bytes.Length, true),
                packet);
        }
    ...
    public void Recv(TransferBuffer raw_buffer)
    {
        ...
        lock (m_class_lock)
        {
            this.PrepareIncomingBuffer(raw_buffer, incoming_buffers_tmp);
            ...
            if (incoming_buffers_tmp.Count > 0)
            {
                foreach (TransferBuffer buffer in incoming_buffers_tmp)
                {
                    ...
                    if (m_client_security)
                    {
                        if (m_security_flags.security_bytes == 1)
                        {
                            byte expected_count = this.GenerateCountByte(true);
                            ...
                            byte expected_crc = this.GenerateCheckByte(buffer.Buffer);
                    ...
                    {
                        this.Handshake(packet_opcode, packet_data, packet_encrypted);
    ...
    private void PrepareIncomingBuffer(TransferBuffer raw_buffer, List<TransferBuffer> incoming_buffers_tmp)
    {
    ...
    public List<KeyValuePair<TransferBuffer, Packet>> TransferOutgoing()
    {
        ...
        lock (m_class_lock)
        {
            if (this.HasPacketToSend())
            {
                ...
                {
                    buffers.Add(this.GetPacketToSend());
                }
            }
        }
