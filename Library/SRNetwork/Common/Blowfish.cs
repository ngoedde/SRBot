...
public sealed class Blowfish
{
    ...
    private readonly uint[] _pArray = new uint[N + 2];
    private readonly uint[,] _sBoxes = new uint[4, 256];
    ...
    public void Initialize(ReadOnlySpan<byte> key)
    {
        if (key.Length < KEY_SIZE_MIN || key.Length > KEY_SIZE_MAX)
            throw new ArgumentOutOfRangeException(nameof(key), $"Must be between {KEY_SIZE_MIN} and {KEY_SIZE_MAX}");

        //for (int i = 0; i < 4; i++)
        //{
        //    for (int ii = 0; ii < 256; ii++)
        //        _sBoxes[i, ii] = _bf_S[i, ii];
        //}
        //Buffer.BlockCopy(_bf_S, 0, _sBoxes, 0, 4 * 256 * sizeof(uint));
    ...
    public byte[]? Encode(byte[] input, int index, int length)
    {
        ...
        if (length <= index)
            throw new ArgumentException("Must be greater than index", nameof(length));
    ...
    {
        var outputLength = GetOutputLength(input.Length);
